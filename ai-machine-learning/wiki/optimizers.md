...
- **Quantified HP interdependence**: across all pairwise HP combinations, mean transfer error (the loss penalty from sweeping one HP at a non-optimal value of another, then transferring) is **0.03 for µP vs. 0.005 for u-µP** — a 6x reduction, which is what makes u-µP's independent 1-D sweeps viable where µP's combined-multiplier sweep causes loss to spike instead.

The concrete FP8 recipe: cast inputs/weights of non-critical matmuls (query/key/value projections, FFN input layer) to **E4M3** and the output gradient to **E5M2**, in both the forward pass and both backward passes; keep the attention dense-projection input, the final FFN matmul input, and the decoder head weight ("critical tensors", identified empirically as the ones that exhibit scale growth during training) in higher precision, along with embeddings, residual adds, and nonlinearities; optimizer states stay FP32. This leaves **about 70% of the transformer block's matmul computation running natively in FP8**. Validated at production scale (300B tokens of SlimPajama, Llama architecture, 1B/3B/7B parameters): FP8 training shows no significant loss degradation versus BF16 at any of the three sizes, and at 7B the FP8 model's zero-shot scores are mostly on par with BF16 across MMLU (31.2 vs 29.0), HellaSwag (53.4 vs 53.4), OpenBookQA (29.6 vs 31.6), PIQA (77.6 vs 77.1), TriviaQA (21.3 vs 23.4), and WinoGrande (65.7 vs 63.7) — both beating the SP baseline's MMLU (29.6) and TriviaQA is the only metric where FP8 trails BF16 by more than a point. At this larger scale the best independent-search HPs shifted non-trivially from the small-scale experiments: global LR multiplier `η = 23.5` and `α_res-attn-ratio = 2^-2.0`, with all other u-µP HPs left at the default of 1 — underscoring that "near-default" doesn't mean "always exactly 1" once you're at frontier scale. One stated limitation: the modified embedding-LR rule is justified only empirically, with no theoretical derivation, and neither µP nor Unit Scaling guarantees tensors stay well-behaved throughout training (the critical-tensor scale growth is observed, not proven bounded).
...
Concretely, relative to standard µP's width-only multipliers (`m_N = N/N_base`), CompleteP adds a depth multiplier `m_L = L/L_base` and scales: pre-LN and hidden-layer learning rates by an extra `m_L^(α-1)` factor (so at `α=1` this term vanishes to 1, while bias and LayerNorm learning rates pick up the same `m_L^(α-1)` factor that hidden weights would otherwise lack); AdamW's `ε` for residual-block parameters by `m_N^-1 · m_L^-α`; and the residual branches themselves by `m_L^-α` as in the equation above. The paper formalizes *why* `α=1` is the unique right choice via three desiderata: (1) **stable initialization** — hidden-layer norms and outputs stay `Θ(1)` as width and depth both go to infinity, which alone only requires `α ≥ 0.5`; (2) **maximal residual-stream update** — each residual block's own weights should contribute `Θ(1/L)` to the change in the stream (matching µP's per-layer update size when `L` is finite), which forces the AdamW LR to scale as `Θ(L^(1-α))` and bounds `α ≤ 1`; (3) their novel **complete feature learning** criterion — no hidden layer or the output should be "lazy" (i.e., statistically indistinguishable from its first-order Taylor linearization around initialization) with respect to any subset of parameters as both `N, L → ∞`. A toy depth-2, width-1 block construction in the paper shows `α = 0.5` networks asymptotically collapse onto their linearization at a rate of `1/sqrt(L)` (becoming "lazy"), while standard SP/µP-with-depth-scaling actually diverges numerically at large depth (NaNs in their experiments) — only `α=1` avoids both failure modes simultaneously.

Reported results, all on Cerebras CS-3 hardware with compute-optimal training (20 tokens-per-parameter):
- CompleteP shows stable optimal LR/init-std across depths 2 to 128 (exceeding LLaMA-70B's 80 layers and LLaMA-405B's 126 layers), where SP, µP, and `α = 0.5` all drift. This was tested both in the "traditional" fixed-token HP-transfer setting (300M tokens, batch size 128, no weight decay) and under the compute-optimal 20-TPP setting with tuned weight decay — the compute-optimal setting is markedly less sensitive to learning rate overall, but CompleteP still wins without any additional retuning from its base-depth value.
...
- CompleteP widens the range of compute-efficient width:depth ratios: at 1.5B non-embedding params, a narrow-deep model with `N:L ≈ 11.8` stays within 1% of compute-optimal loss under CompleteP, versus `N:L ≈ 38.7` for µP — relevant for low-memory hardware that streams one layer at a time. Compute-optimal `N:L` itself trends larger as model scale increases for every parameterization tested, but stays under ≈60 across the 50M–1.5B range studied — a useful corrective to Kaplan et al.'s `N:L ≈ 100` rule of thumb, which the authors argue was distorted by testing under SP/µP where deep models are unfairly hyperparameter-detuned.
- Downstream zero-shot evals (HellaSwag, ARC-Easy, LAMBADA, RACE, PIQA, BoolQ) at 1.5B confirm the upstream loss gains transfer to task accuracy, not just validation perplexity: at the compute-optimal shape CompleteP's downstream average is 55.2% vs. 54.3% for µP and α=0.5 (tied), and at the deepest 179-layer shape CompleteP holds 54.3% downstream average vs. 52.0% (µP) and 50.1% (α=0.5) — the downstream gap widens with depth just like the upstream FLOP-savings gap.
- The result isn't an artifact of standard 20-TPP training only: re-run at **200 tokens-per-parameter** (where models are more over-trained, a regime increasingly used for parameter-efficient deployment), CompleteP still gives the best validation loss at both 50M and 300M non-embedding scales, at both the compute-optimal and deepest width:depth ratios tested, though the margin over µP narrows (e.g. 2.859 vs. 2.867 at 50M-optimal, 2.497 vs. 2.537 at 300M-deepest).

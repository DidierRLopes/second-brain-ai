...
Using Gemini-2.5-Flash as both the harness-synthesizing model and the policy, AutoHarness reaches a **100% legal-move success rate across all 145 TextArena games tested** (1- and 2-player, including Chess, Checkers, Blackjack, Sudoku, and novel variants), after a median of ~14.5 tree-search iterations (19/32 evaluated games converge in under 10 iterations). The hardest games to learn — needing the most LLM calls/refinement rounds — were GermanWhist-v0 (2P), Cryptarithm-v0 (1P), Othello-v0 (2P), and Chess-v0 (2P). Training deliberately strips "Available Moves" hints from the observation string first, forcing the harness to infer legality from environment feedback rather than copying a provided list — the authors note this makes the problem strictly harder than necessary but more realistic. With the harness in place, the much smaller Gemini-2.5-Flash beats the larger Gemini-2.5-Pro on 9/16 two-player games (56.3% win rate vs. 38.2%; rising to 64.8%/12-of-16 against vanilla Flash) and matches or beats it on 1-player games (0.745 average reward vs. 0.707 for Gemini-2.5-Pro and 0.673 for vanilla Flash, winning 8/16 and tying 5/16).

Pushed to harness-as-policy on 16 single-player games, the training objective changes to a sparse-reward-aware heuristic — H=0 if an illegal action is taken, H=0.5+0.5r otherwise, where r is the (only-available-at-episode-end) environment reward — trained for up to 256 iterations, averaging 89.4 iterations to reach a heuristic value of 0.939. The resulting pure-code policy (no LLM call at inference) reaches 0.870 average reward — beating GPT-5.2-High (0.844), Gemini-2.5-Pro (0.707), and GPT-5.2 (0.635) — winning 3/16 games outright (vs. GPT-5.2-High's 5/16, tying the remaining 8/16), at near-zero inference cost versus roughly $640 for the GPT-5.2 and GPT-5.2-High evaluation runs combined. The headline result: a smaller model synthesizing a custom code harness can outperform a much larger model used directly, while being far cheaper to run. The authors explicitly scope harness-as-policy to 1-player games only, noting 2-player games would require MCTS-like opponent modeling and a learned code world model (as in their companion "Code World Models for General Game Playing" work) to search over — a harder problem they leave to future work, alongside distilling the per-game harnesses back into the base LLM for recursive self-improvement and building a reusable harness library.
...
Results across three domains: on online text classification (GPT-OSS-120B), Meta-Harness-discovered harnesses beat Agentic Context Engineering (ACE) by **7.7 points while using 4× fewer context tokens** (48.6% vs. ACE's 40.9%, using 11.4K context tokens vs. ACE's 50.8K), and beat Meta Context Engineering (MCE) by 8.6 points, while matching the next-best optimizer's final accuracy after only 4 evaluations vs. that method's full budget. Head-to-head against other text optimizers under matched proposer/budget (Table 4), Meta-Harness reaches 50.0 median / 56.7 best accuracy, vs. GEPA's 32.6/40.2, Best-of-N's 34.0/44.2, OpenEvolve's 39.1/43.3, and TTT-Discover's 34.1/45.6 — more than 10 points above the next-best method, in roughly 0.1× the evaluations. An **information-access ablation** isolates why: a scores-only proposer interface reaches 34.6 median / 41.3 best accuracy, a scores-plus-LLM-summary interface reaches 34.9/38.7 (summaries don't help, and may even hurt by compressing away diagnostic detail), while the full interface with raw execution traces reaches 50.0/56.7 — evidence that unsummarized access to prior code and traces, not just more search iterations, is the active ingredient. On 9 out-of-distribution text-classification datasets unseen during search, the selected harness generalizes with 73.1% average accuracy (best on 6/9 datasets) vs. ACE's 70.2%, and the paper notes few-shot prompting beyond 32 examples actually *hurts* accuracy on 7/9 of these OOD tasks. On retrieval-augmented math reasoning, a single discovered harness improves accuracy on 200 IMO-level problems by **4.7 points on average across five held-out models** (i.e., the discovered harness generalizes across models, not just the one it was optimized against), beating BM25 retrieval by 1.3 points overall while avoiding the regressions dense retrieval and random few-shot prompting show on several models. On agentic coding (TerminalBench-2), the discovered harness — built on top of Terminus-KIRA's native tool calling and completion checklist — surpasses the Terminus-KIRA baseline (76.4% vs. 74.7% pass rate on Opus 4.6) and ranks #2 among all Opus 4.6 agents on the public leaderboard (behind only ForgeCode at 81.8%, whose result the authors could not reproduce from the public repo), and #1 among all Claude Haiku 4.5 agents (37.6% vs. runner-up Goose's 35.5%).

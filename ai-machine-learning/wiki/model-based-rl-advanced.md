...
**Results.** Combined with multiple state-of-the-art value-based methods across online and offline-to-online benchmarks (OGBench locomotion/manipulation, RoboMimic), LQL consistently outperforms both 1-step TD and n-step TD at similar runtime cost. On `humanoidmaze-giant` — the longest-horizon task in OGBench — LQL with trajectory length 64 scales cleanly where n-step TD (tested at n = 4, 8, 16, 64) degrades as n increases, confirming that the inequality-based backstop, not merely "use more steps," is what fixes long-horizon brittleness.

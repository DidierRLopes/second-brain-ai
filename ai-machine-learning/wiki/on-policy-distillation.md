...
**Results.** On Qwen3-4B trained for 400 steps on DAPO-Math-17k (privileged hints generated by Gemini 2.5 Pro), both SDPG variants beat GRPO and RLSD on every benchmark: SDPG-UFKL reaches AIME24 best=0.408 / AIME25 best=0.335 / AMC23 best=0.870 (mean@32, pass@1), versus GRPO's 0.316 / 0.279 / 0.739 and RLSD's 0.395 / 0.304 / 0.813. SDPG-UFKL takes the top score on five of six (Last/Best × 3 benchmarks) metrics, SDPG-URKL the remaining one. The accuracy gap over GRPO opens within the first 50 steps and persists for the full 400-step run, and SDPG reaches its high-reward plateau several hundred steps earlier than GRPO. The most diagnostic result is on training stability: **RLSD's actor entropy collapses toward zero by step 250** — a known signature of mode collapse from pure self-distillation — while **SDPG-UFKL maintains substantially higher entropy throughout training**, which the authors attribute to positive-advantage gating plus the warmup-decay β schedule jointly preventing the privileged signal from over-constraining the policy once the student has absorbed its useful content. SDPG response lengths stabilize at intermediate values (sufficient for multi-step reasoning, shorter than GRPO's verbose outputs).
